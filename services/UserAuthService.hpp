...
public:
  UserAuthService(mongocxx::collection coll, mongocxx::collection e_coll)
      : collection(coll), email_collection(e_coll) {}

  // ------------------------------------------------------------------
  // Register a new user
  // ------------------------------------------------------------------
  bool registerUser(const std::string &email, const std::string &password) {
    std::cerr << "DEBUG: registerUser start" << std::endl;
    if (email.empty() || password.empty()) {
      return false;
    }

    // Check if email already exists
    std::cerr << "DEBUG: checking existing" << std::endl;
    auto existing =
        email_collection.find_one(make_document(kvp("email", email)));
    if (existing) {
      std::cerr << "DEBUG: email exists" << std::endl;
  ...
  // ------------------------------------------------------------------
  bool verifyPassword(const std::string &email,
                      const std::string &passwordAttempt) {
