...
public:
  ...
  json login(const std::string &email, const std::string &password) {
    // ===== ADMIN LOGIN =====
    if (email == "admin@gmail.com" && password == "A!1111") {
      std::string adminToken = JWT::createToken("admin", "admin");

      return {
          {"role", "admin"},
          {"token", adminToken},
          {"admin", {{"email", "admin@gmail.com"}, {"name", "Administrator"}}}};
    }
    ...
    if (user) {
      ...
      return {{"role", "user"}, {"token", token}, {"user", cleanUser}};
    }
    ...
    if (company) {
      ...
      return {{"role", "company"}, {"token", token}, {"company", cleanCompany}};
    }
