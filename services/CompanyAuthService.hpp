...
public:
    CompanyAuthService(mongocxx::collection coll, mongocxx::collection e_coll) : collection(coll), email_collection(e_coll) {}
    bool registerCompany(const std::string &email, const std::string &password)
    {
        ...
        Company company(email, hashed);
        collection.insert_one(company.toBson().view());
