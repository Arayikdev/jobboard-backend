...
public:
  CompanyService(mongocxx::collection db, mongocxx::collection j)
      : companycollection(db), jobcollection(j) {}
  ...
  json getPublicCompany(const bsoncxx::oid &companyId) {
    auto result =
        companycollection.find_one(bsoncxx::builder::basic::make_document(
  ...
  json updateCompanyProfile(const bsoncxx::oid &companyId, const json &body) {
    ...
    // 1) Найти компанию
    auto existing =
        companycollection.find_one(make_document(kvp("_id", companyId)));
